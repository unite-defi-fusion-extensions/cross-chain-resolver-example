...
describe('Resolving example', () => {
    ...
    it.skip('should swap Ethereum USDC -> Bsc USDC. Single fill only', async () => {
        ...
        // user shares secret when block with deployed DstEscrow finalized
    ...
    it('should swap Ethereum USDC -> Bsc USDC. Multiple fills. Fill 100%', async () => {
        ...
        // User creates order
        // 11 secrets s
        const secrets = Array.from({length: 11}).map(() => uint8ArrayToHex(randomBytes(32))) // note: use crypto secure random number in real world
        ...
        const {txHash: orderFillHash, blockHash: srcDeployBlock} = await srcChainResolver.send(
            resolverContract.deploySrc(
                ...
                Sdk.TakerTraits.default()
                    ...
                    .setAmountThreshold(order.takingAmount),
                fillAmount,
                Sdk.HashLock.fromString(leaves[idx])
            )
        )
