...
describe('1inch Fusion Swap: EVM to a REAL Lightning Node', () => {
    ...
    beforeEach(async () => {
        ...
        await user.topUpFromDonor(config.chain.source.tokens.USDC.address, config.chain.source.tokens.USDC.donor, parseUnits('1000', 6));
        await resolver.topUpFromDonor(getAddress('0x0000000000000000000000000000000000000000'), getAddress('0x00000000219ab540356cBB839Cbe05303d7705Fa'), parseEther('10'));
        await user.approveToken(config.chain.source.tokens.USDC.address, config.chain.source.limitOrderProtocol, MaxUint256);
    ...
    it('should swap User:USDC for Resolver:LightningSats via a real LND node', async () => {
        ...
        const order = Sdk.CrossChainOrder.new(
            ...
            { hashLock, timeLocks: Sdk.TimeLocks.new({ srcWithdrawal: 10n, srcPublicWithdrawal: 7200n, srcCancellation: 7201n, srcPublicCancellation: 7202n, dstWithdrawal: 10n, dstPublicWithdrawal: 3600n, dstCancellation: 3601n, }), srcChainId: src.chainId, dstChainId: Sdk.NetworkEnum.BINANCE, srcSafetyDeposit: parseEther('0.01'), dstSafetyDeposit: 0n },
            { auction: new Sdk.AuctionDetails({ startTime: srcTimestamp, duration: 120n, initialRateBump: 0, points: [] }), whitelist: [ { address: new Sdk.Address(src.resolver), allowFrom: 0n } ], resolvingStartTime: 0n },
        ...
        const { blockHash: srcDeployBlock } = await resolver.send(
            resolverContract.deploySrc(src.chainId, order, signature, Sdk.TakerTraits.default().setExtension(order.extension).setAmountMode(Sdk.AmountMode.maker), order.makingAmount)
        );
